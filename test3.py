...
class ImageLabel(QLabel):
    def __init__(self, parent=None):
        ...
        self.drawing_arrow = False  # To indicate if an arrow is being drawn
    ...
    def mousePressEvent(self, event):
        if self.pixmap_loaded:
            if event.button() == Qt.RightButton:
                # Right-click clears the points and stops drawing the arrow
                self.points.clear()
                ...
                if len(self.points) < 2:
                    ...
                    if len(self.points) == 1:
                        # Start drawing the arrow after first point is clicked
                        self.drawing_arrow = True
                if len(self.points) == 2:
                    # Stop drawing the arrow and trigger a repaint for the line
                    self.drawing_arrow = False
                    self.update()

    def mouseMoveEvent(self, event):
        if self.pixmap_loaded and self.drawing_arrow and len(self.points) == 1:
    ...
    def paintEvent(self, event):
        ...
        if self.drawing_arrow and len(self.points) == 1 and self.temp_point:
            # If one point is selected and the arrow is being drawn, show the arrow
            p1 = self.points[0]
            p2 = self.temp_point
